...
def dump_packet(data): 
 ...
 for d in dump_data:
  print(' '.join("{:02X}".format(byte2int(x))for x in d)+'   '*(16-len(d))+' '*2+''.join(printable(x)for x in d))
 print("-"*66)
 print()
...
class FieldDescriptorPacket(MysqlPacket):
 ...
 def description(self):
  return(self.name,self.type_code,None,self.get_column_length(),self.get_column_length(),self.scale,self.flags%2==0)
 def get_column_length(self):
  ...
  return self.length
 def __str__(self):
  return('%s %r.%r.%r, type=%s, flags=%x'%(self.__class__,self.db,self.table_name,self.name,self.type_code,self.flags))
...
class EOFPacketWrapper(object):
 def __init__(self,from_packet):
  if not from_packet.is_eof_packet():
   raise ValueError("Cannot create '{0}' object from invalid packet type".format(self.__class__))
  self.packet=from_packet
